...
namespace PotatoesSoup {
	public class Akimbo {
		...
		public static bool Handgun_UpdateInteraction_AkimboReloading(Handgun __instance, ref FVRViveHand hand)
		{
			...
			if (BepInExPlugin.AkimboOneHand_IsEnabled.Value || (hand.OtherHand.CurrentInteractable != null && hand.OtherHand.CurrentInteractable is Handgun))
			{
				FVRFireArmMagazine nmag;
				if (mag.m_isSpawnLock)
				{
					nmag = UnityEngine.Object.Instantiate<GameObject>(mag.ObjectWrapper.GetGameObject(), mag.Transform.position, mag.Transform.rotation).GetComponent<FVRFireArmMagazine>();
				}
				else
				{
					nmag = mag;
